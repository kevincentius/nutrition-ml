...
class DataSet(object):
    ...
    ROOT_FOLDER = 'D:/master project/data-v2'

    def __init__(self, set_name):
        # set paths
        self.path = self.ROOT_FOLDER + '/' + set_name
    ...
    def save_feature_matrix(self, mat, version):
        with open('{}/{}.csv'.format(self.feature_path, version), 'wb') as file:
            np.savetxt(file, mat)
        
    # if does not exists, returns None
    def load_feature_matrix(self, version):
        path = '{}/{}.csv'.format(self.feature_path, version)
